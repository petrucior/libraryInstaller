...
sudo apt-get -y install python-dev python-numpy libtbb2 libtbb-dev libjpeg-dev libpng-dev libtiff-dev libjasper-dev libdc1394-22-dev libtbb-dev
...
# Configuring OpenCV
if [ $1 == "none" ]; then
    cmake -D CMAKE_BUILD_TYPE=RELEASE -D OPENCV_EXTRA_MODULES_PATH=~/opencv_contrib/modules ~/opencv/ -D CMAKE_INSTALL_PREFIX=/usr/local -D WITH_TBB=ON -D ENABLE_PRECOMPILED_HEADERS=OFF -D OPENCV_ENABLE_NONFREE=ON -D BUILD_NEW_PYTHON_SUPPORT=ON -D WITH_V4L=ON -D WITH_QT=ON -D ENABLE_FAST_MATH=ON -D BUILD_TBB=ON -D ENABLE_SSE42=ON -D ENABLE_SSSE3=ON -D WITH_OPENMP=ON -D WITH_OPENGL=ON BUILD_EXAMPLES=ON ..
fi
...
if [ $1 == "cuda" ]; then
	...
	then echo "64 bits";
	     ...
	     cmake -D CMAKE_BUILD_TYPE=RELEASE -D OPENCV_EXTRA_MODULES_PATH=~/opencv_contrib/modules ~/opencv/ -D CMAKE_INSTALL_PREFIX=/usr/local -D WITH_TBB=ON -D ENABLE_PRECOMPILED_HEADERS=ON -D OPENCV_ENABLE_NONFREE=ON -D BUILD_NEW_PYTHON_SUPPORT=ON -D WITH_V4L=ON -D WITH_QT=ON -D ENABLE_FAST_MATH=ON -D BUILD_TBB=ON -D ENABLE_SSE42=ON -D ENABLE_SSSE3=ON -D WITH_OPENMP=ON -D WITH_OPENGL=ON BUILD_EXAMPLES=ON -D WITH_CUDA=ON -D WITH_CUBLAS=ON ..
...
if [ $1 == "opencl" ]; then
    ...
    cmake -D CMAKE_BUILD_TYPE=RELEASE -D OPENCV_EXTRA_MODULES_PATH=~/opencv_contrib/modules ~/opencv/ -D CMAKE_INSTALL_PREFIX=/usr/local -D WITH_TBB=ON -D ENABLE_PRECOMPILED_HEADERS=ON -D OPENCV_ENABLE_NONFREE=ON -D BUILD_NEW_PYTHON_SUPPORT=ON -D WITH_V4L=ON -D WITH_QT=ON -D ENABLE_FAST_MATH=ON -D BUILD_TBB=ON -D ENABLE_SSE42=ON -D ENABLE_SSSE3=ON -D WITH_OPENMP=ON -D WITH_OPENGL=ON BUILD_EXAMPLES=ON -D WITH_OPENCL=ON -D WITH_OPENCLAMDFFT=ON -D WITH_OPENCLAMDBLAS=ON ..
fi
